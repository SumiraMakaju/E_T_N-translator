...
async function doTranslate() {
  ...
  if (srcSelect.value === tgtSelect.value) {
    showOutput("⚠ Source and target must differ.", true);
    return;
  }
  ...
  chrome.runtime.sendMessage(
    ...
    (response) => {
      ...
      if (response && response.success) {
        outputLangLabel.textContent = LANG_NAMES[tgtSelect.value] || tgtSelect.value;
        showOutput(response.output, false);
        loadHistory();
      } else if (response && response.error === "NO_API_KEY") {
        showOutput("API key not set. Click the settings icon to configure.", true);
        noKeyBanner.style.display = "flex";
      } else {
        showOutput((response && response.error) || "Translation failed. Try again.", true);
      }
    }
  );
}
...
async function loadHistory() {
  ...
  historyList.querySelectorAll(".history-item").forEach((el, i) => {
    el.addEventListener("click", () => {
      ...
      outputLangLabel.textContent = LANG_NAMES[item.tgt_lang] || item.tgt_lang;
    });
  });
}
